...
@Service
public class VtsJsonServiceImpl implements VtsJsonService {
    ...
    @Override
    public List<VtsPacketDto> parsePacketList(List<String> rawPackets) {
        return rawPackets.stream()
                .map(VtsPacketParser::parseRawPacket)
                .filter(Objects::nonNull)
                .toList();
    }

    @Override
    public List<VtsPacketDto> getPacketsFromDatabase(String imei, String vehicleNo) {
        List<VehicleRouteData> routePoints = repository.findAllRouteData();
        List<VtsPacketDto> dtoList = new ArrayList<>();
        ...
        for (VehicleRouteData point : routePoints) {
            ...
            VtsPacketDto dto = VtsPacketParser.parseRawPacket(rawPacket);
            if (dto != null) {
                dtoList.add(dto);
            }
        }
